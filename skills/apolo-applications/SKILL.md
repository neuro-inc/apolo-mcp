...
6. Describe the exact reviewed operation, then invoke it so the MCP host can apply its
   approval UI. Require `managed` or `full` server policy and the exact unexpired
   single-use plan. Recheck checksum, parsed YAML, context, template, and current
   revision. Reject edits or drift and create a fresh plan.
